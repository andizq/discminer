...
class GridTools:
    ...
    @staticmethod
    def get_sky_from_disc_coords(R, az, z, incl, PA, xc=0, yc=0, midplane=False, return_rpa=False):
        xp = R*np.cos(az)
        yp = R*np.sin(az)
        if midplane:
            zp = 0.0
        ...
        if return_rpa:
            PAsky = np.arctan2(yp+yc, xp+xc) - np.pi/2
            rsky = np.hypot(xp+xc, yp+yc)
            return rsky, PAsky #in au, radians
