...
plot_channels = np.linspace(-idlim,idlim,nchans) + np.argmin(np.abs(datacube.vchannels-params['velocity']['vsys']))
...
mark_planet_location(ax, args, edgecolors='limegreen', lw=4.5, s=650, alpha=0.8, coords='disc', zfunc=z_func, zpars=z_pars, incl=incl, PA=PA, xc=xc, yc=yc, dpc=dpc, kwargs_text=dict(fontsize=20, color='limegreen'))        
